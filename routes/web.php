...
Route::middleware(['auth', 'verified', 'route_access'])->group(function () {
    ...
    Route::controller(BookFrontController::class)->prefix('books')->name('front.books.')->group(function () {
        Route::get('/', 'index')->name('index');
        Route::get('/{book:slug}', 'show')->name('show');
    });

    Route::controller(CategoryFrontController::class)->prefix('categories')->name('front.categories.')->group(function () {
        Route::get('/', 'index')->name('index');
        Route::get('/{category:slug}', 'show')->name('show');
    });

    Route::controller(LoanFrontController::class)->prefix('loans')->name('front.loans.')->group(function () {
        Route::get('/', 'index')->name('index');
        Route::get('/{loan:loan_code}/detail', 'show')->name('show');
        Route::post('/{book:slug}/create', 'store')->name('store');
    });

    Route::controller(ReturnBookFrontController::class)->prefix('return-books')->name('front.return-books.')->group(function () {
        Route::get('/', 'index')->name('index');
        Route::get('/{returnBook:return_code}/detail', 'show')->name('show');
        Route::post('/{book:slug}/create/{loan:loan_code}', 'store')->name('store');
    });

    Route::get('fines', FineFrontController::class)->name('front.fines.index');
});
